  ...
  try {
    if (tokenMintStr === 'So11111111111111111111111111111111111111112') {
      ...
      try {
        ...
        wsolBalRaw = Number(balInfo.value.amount);
      } catch (e) {}
...
async function getAmmLiquidityAnalysis(mint) {
  try {
    ...
    try {
      const connection = new Connection(appConfig.solanaRpcUrl || process.env.SOLANA_RPC_URL || 'https://api.mainnet-beta.solana.com', { commitment: 'confirmed', disableRetryOnRateLimit: true });
