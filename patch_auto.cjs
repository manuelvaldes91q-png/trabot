...
const targetAutoSol = `        // Verifica si el precio bajó hasta el punto de entrada
        if (cp <= o.price * 1.005) {
          o.status = 'filled'; 
          o.filledAt = Date.now(); 
          o.filledPrice = cp;
...
const replaceAutoSol = `        // Verifica si el precio bajó hasta el punto de entrada
        if (cp <= o.price * 1.005) {
          o.status = 'filled'; 
          o.filledAt = Date.now(); 
...
const targetQMB = `        w.orders.push(order);
...
const replaceQMB = `        w.orders.push(order);
...
server = server.replace(targetAutoSol, replaceAutoSol);
server = server.replace(targetQMB, replaceQMB);
